"""
YOLOv8n Grape Bunch Detection with Hailo8L AI Acceleration Module
================================================================

This module implements real-time grape bunch detection using YOLOv8n neural networks
optimized with Hailo8L AI hardware acceleration. It provides comprehensive computer vision
...
.. module:: grape_bunch_hailo8l
    :synopsis: YOLOv8n grape bunch detection with Hailo8L AI acceleration for precision viticulture
...
Detection Pipeline:
    Image Input → Preprocessing → YOLOv8n Inference → Hailo8L Acceleration → Detection Extraction → Visualization
...
Model Dependencies:
    - **grape_bunch_hailo8l.hef**: Compiled YOLOv8n model for Hailo8L hardware
...
Recovery Mechanisms:
    ...
    - **Model Recovery**: Hailo8L AI initialization error handling and retry logic
...
.. code-block:: bash
    ...
    python grape_bunch_hailo8l.py -i vineyard_image.jpg --output_folder results/
...
- **Model Dependencies**: Depends on grape_bunch_hailo8l.hef model file and label definitions
...
def extract_detections(hailo_output: list, w: int, h: int, class_names: list, threshold: float = 0.5) -> list:
    """
    Extract and filter grape bunch detections from Hailo8L AI model output.
    ...
    :param hailo_output: Raw output from Hailo8L AI YOLOv8n model inference
    ...
    :param threshold: Minimum confidence score for detection validation (default: 0.5)
    ...
    Workflow
    --------
    1. **Output Processing**: Iterate through Hailo8L AI model detection results
    ...
    .. note::
        The function expects Hailo8L AI output in YOLOv8n format with normalized
    ...
    # Iterate through each object class in the Hailo8L AI model output
...
def grape_bunch_run(input: str, output_folder: str) -> dict:
    """
    Execute comprehensive grape bunch detection on agricultural images using YOLOv8n with Hailo8L AI acceleration.
    ...
    YOLOv8n neural network capabilities with Hailo8L AI hardware acceleration to
    ...
    :raises RuntimeError: When Hailo8L AI model loading or inference fails
    ...
    5. **Model Initialization**: Load and initialize Hailo8L AI YOLOv8n detection model
    ...
    AI Processing Pipeline:
        - **Model Loading**: Hailo8L AI runtime initialization with YOLOv8n model
    ...
    Detection Configuration:
        ...
        - **Hardware Acceleration**: Hailo8L AI inference optimization
    ...
    .. note::
        ...
        corresponding class label definitions. Ensure Hailo8L AI hardware
    ...
    Notes
    -----
    - **Hardware Requirements**: Requires compatible Hailo8L AI acceleration hardware
    - **Model Dependencies**: Depends on grape_bunch_hailo8l.hef and label files
    ...
    with open("Images/Models/Grape_Bunch_Hailo8L/grape_bunch_labels.txt", 'r', encoding="utf-8") as f:
    ...
    # Initialize Hailo8L AI context manager for YOLOv8n model processing
    # This loads the compiled neural network model optimized for Hailo8L hardware
    with Hailo("Images/Models/Grape_Bunch_Hailo8L/grape_bunch_hailo8l.hef") as hailo:
        ...
        # Execute YOLOv8n inference with Hailo8L AI hardware acceleration
        ...
        # Time
        record_model_execution("Grape_Bunch_Hailo8L", inference_time, input)
...
def record_model_execution(model_name: str, execution_time: float, image: str):
    """Time execution"""
    log_file = "Images/analyze_time_mission1.json"
    data = {
...
if __name__ == "__main__":
    """
    Command-line interface for grape bunch detection processing with Hailo8L acceleration.
    ...
    The interface leverages Hailo8L AI hardware acceleration for optimal performance.
    
    The interface supports agricultural field operations by providing simple
    command-line access to YOLOv8n grape detection with Hailo8L AI acceleration,
    ...
    Usage Examples:
        ...
        python grape_bunch_hailo8l.py -i Images/Camera/section_A.jpg --output_folder Images/Results/section_A/
        
        # Batch processing integration (external script)
        for image in vineyard_images/*; do
            python grape_bunch_hailo8l.py -i "$image" --output_folder "results/$(basename "$image" .jpg)/"
        done
    ...
    parser = argparse.ArgumentParser(
        description="Run YOLOv8n grape bunch detection with Hailo8L AI acceleration",
        epilog="Example: python grape_bunch_hailo8l.py -i vineyard.jpg --output_folder results/"
    )
