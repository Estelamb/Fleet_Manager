...
"""
Grape Leaf Detection and Extraction with Hailo8 AI Acceleration Module
======================================================================

This module implements comprehensive grape leaf detection and extraction using
YOLOv8n neural networks optimized with Hailo8 AI hardware acceleration. It provides
...
.. module:: leaf_detection_hailo8
    :synopsis: YOLOv8n grape leaf detection with Hailo8 acceleration for agricultural preprocessing
...
Detection Pipeline:
    Image Input → Preprocessing → YOLOv8n Inference → Hailo8 Acceleration → Leaf Extraction → Crop Generation
...
Processing Workflow:
    ...
    3. **AI Inference**: YOLOv8n neural network execution with Hailo8 acceleration
...
Grape Leaf Detection and Extraction Pipeline:
    ...
    2. **Model Initialization**: Hailo8 AI runtime and YOLOv8n model loading
...
AI Model Processing:
    ...
    - **Hailo8 Optimization**: Hardware-specific model optimization and acceleration
...
Model Dependencies:
    - **leaf_detection_hailo8.hef**: Compiled YOLOv8n model for Hailo8 hardware
...
Detection Performance:
    ...
    - **Low Latency**: Hailo8 AI acceleration for responsive processing
...
Multi-level Error Management:
    ...
    - **Model Loading**: Hailo8 AI model loading and initialization error handling
...
.. code-block:: bash
    ...
    python leaf_detection_hailo8.py -i vineyard_image.jpg
...
- **Model Dependencies**: Depends on leaf_detection_hailo8.hef model and label files
...
def leaf_detection_run(input_image_path: str, score_threshold: float = 0.25) -> str:
    """
    Execute comprehensive grape leaf detection and extraction using YOLOv8n with Hailo8 AI acceleration.
    ...
    architecture optimized with Hailo8 hardware acceleration for real-time grape leaf
    ...
    Workflow:
        ...
        3. **Model Initialization**: Initialize Hailo8 AI accelerator with YOLOv8n model
    ...
    Notes
    -----
    - **Hardware Acceleration**: Requires Hailo8 AI accelerator for optimal performance
    - **Model Dependencies**: Requires leaf_detection_hailo8.hef and leaf_detection_labels.txt
    ...
    See Also
    --------
    extract_detections : Extract detection results from Hailo8 neural network output
    ...
    with open("Images/Models/Grape_Diseases_Hailo8/Leaf_Detection/leaf_detection_labels.txt", 'r', encoding="utf-8") as f:
    ...
    # Initialize Hailo8 AI accelerator with leaf detection model
    # Uses context manager for proper resource cleanup
    with Hailo("Images/Models/Grape_Diseases_Hailo8/Leaf_Detection/leaf_detection_hailo8.hef") as hailo:
        ...
        cv2.imwrite(temp_file, resized_image_rgb)
        ...
        # Inference time
        record_model_execution("Leaf_Detection_Hailo8", inference_time, input_image_path)
        ...
        output_dir = os.path.join("Images", "Models", "Grape_Diseases_Hailo8", "Leafs", os.path.splitext(os.path.basename(input_image_path))[0])
...
def extract_detections(hailo_output: list, w: int, h: int, class_names: list, threshold: float = 0.5) -> list:
    """
    Extract and process leaf detections from Hailo8 neural network output with confidence filtering.
    ...
    ----------
    hailo_output : list
        Raw output tensor from Hailo8 neural network inference.
...
if __name__ == "__main__":
    ...
    Command-line Usage:
        python leaf_detection_hailo8.py -i <input_image_path>
    ...
    Examples:
        ...
        python leaf_detection_hailo8.py --input field_survey_1.jpg
        
        # Batch processing with shell script
        for img in *.jpg; do
            python leaf_detection_hailo8.py -i "$img"
        done
    ...
    parser = argparse.ArgumentParser(
        description="Run grape leaf detection and extraction using YOLOv8n with Hailo8 AI acceleration",
        epilog="Example: python leaf_detection_hailo8.py -i vineyard_section_1.jpg",
